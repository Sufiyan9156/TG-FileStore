...
# 🔥 RANDOM DAILY FORCE SUB SYSTEM (JOJO SYNC)

import random
from datetime import datetime
from database.database import db

async def get_random_fsubs(user_id, all_channels):

    user_data = await db.users.find_one({"_id": user_id})

    today = datetime.utcnow().date().isoformat()

    # Reset daily assignment
    if not user_data or user_data.get("fsub_date") != today:
        random_count = random.choice([3, 4, 5])
        selected = random.sample(all_channels, min(random_count, len(all_channels)))

        await db.users.update_one(
            {"_id": user_id},
            {"$set": {"fsub_date": today, "assigned_fsubs": selected}},
            upsert=True
        )

        return selected

    return user_data.get("assigned_fsubs", [])
